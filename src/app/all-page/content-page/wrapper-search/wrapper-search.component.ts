...
export class WrapperSearchComponent implements OnInit {
  ...
  searchUsers(keyword: string): void {
    this.userService.searchUsers(keyword).subscribe(
      (data) => {
        // console.log('Dữ liệu người dùng:', data); // Thêm log để kiểm tra dữ liệu
        if (Array.isArray(data)) {
          this.users = data; // Lưu dữ liệu người dùng
          this.noUsersFound = this.users.length === 0; // Kiểm tra xem có người dùng nào không
        } else {
          // console.error('Dữ liệu trả về không phải là mảng:', data);
          this.users = []; // Gán giá trị rỗng để tránh lỗi *ngFor
          this.noUsersFound = true; // Đánh dấu là không tìm thấy người dùng
        }
      },
      (error) => {
        // console.error('Có lỗi xảy ra!', error);
        this.noUsersFound = true; // Có thể đánh dấu không tìm thấy nếu có lỗi
      }
    );
  }
